    ...
    public function handle()
    {
        ...
        foreach ($traders as $trader) {
            ...
            foreach ($data->otherPositionRetList as $respPosition) {
                // Position ids of just updated and created positions
                $updatedAndCreatedIds = [];
                ...
                // New position
                    ...
                    $position->opened_at = Carbon::createFromTimestampMs($respPosition->updateTimeStamp);
                }

                $position->save();

                $updatedAndCreatedIds[] = $position->refresh()->id;

                // Close positions for current trader,
                // which do not exist in the list obtained from the API
                Position::whereNotIn('id', $updatedAndCreatedIds)
                    ->where(['closed_at' => null, 'trader_id' => $trader->id])
